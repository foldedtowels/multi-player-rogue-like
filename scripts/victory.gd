...
## Victory screen shown when all 5 bosses are defeated
...
func _ready():
	...
	# Play toilet flush SFX after a short delay
	await get_tree().create_timer(0.5).timeout
	if audio_manager:
		audio_manager.play_sfx("toilet_flush")

func _setup_ui():
	# Background
	var bg = ColorRect.new()
	bg.color = Color(0.05, 0.1, 0.05, 1)  # Dark green tint
	...
	message.text = "All bosses have been defeated!"
	message.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	message.add_theme_font_size_override("font_size", 24)
	message.add_theme_color_override("font_color", Color(0.7, 0.9, 0.7))
