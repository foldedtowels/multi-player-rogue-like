extends SceneTree
## CLI Test Runner - Comprehensive test suite for Deck Masters
##
## Usage:
...
##   godot --headless --script scripts/tests/run_tests.gd -- --verbose --fail-fast
...
func _init():
	...
	# Get autoloads
	game_manager = root.get_node_or_null("/root/GameManager")
	card_db = root.get_node_or_null("/root/CardDatabase")
	boss_db = root.get_node_or_null("/root/BossDatabase")
...
Usage:
  godot --headless --script scripts/tests/run_tests.gd [options]
